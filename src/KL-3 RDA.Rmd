...
# Permutated genome-wide null distribution for cumulative importance of traits in RDA space
```{r}
K=4
for (i in 1:999){
  print(i)
  G_rand_subset <- G_rand[,sample(1:ncol(G_rand),10000)]
  if(!(dim(G_rand_subset[samprows,])[1]==dim(samp_full5_gen)[1])){
  print("Error dim don't line up")
  break
  }# make sure labels line up
if(!identical(rownames(G_rand_subset[samprows,]),samp_full5_gen$label)){
