...
if (i == 4){
  ...
  #  * Cluster 2 is heterokaryotypes from Maine - "HET-GoM"
  #  * Cluster 3 is homozygous for the reference arrangement from Iceland "REF-Iceland"
  #  * Cluster 4 is homozygous for the alternate arrangement from Maine - "ALT-GoM"
...
### Plot the FST values ####
    ...
    abline(v=breakpoints$LGstartPos[i], col="cornflowerblue", lwd=4)
    abline(v=breakpoints$LGendPos[i], col="cornflowerblue", lwd=4)
    dev.off()
...
## Make full heatmap for LG7 with 4000 random loci, 100 loci at each end, and outliers
...
samp_full <- samp_full[order(samp_full$Ecotype,
                             samp_full$Region,
                             samp_full$PopID),]

samp_inv <- samp_full[,grep("cluster_name", colnames(samp_full))]
head(samp_inv)
rownames(samp_inv) <- samp_full$label
samp_inv$label <- samp_full$label
...
ggplot(samp_inv_melt,  aes(variable, label)) + 
  ...
  scale_fill_manual(values=c("firebrick3", "red",  "grey40", "grey60", "cornflowerblue", "blue" )) +
  theme(axis.text.y=element_text(size=2))
dev.off()
```


```{r plot multi-inversion genotypes v2}

samp_full <- samp_full[order(samp_full$Region,
                        samp_full$Ecotype,
                        samp_full$PopID,
                        samp_full$label),]
...
colnames(samp_inv) <- gsub("cluster_name","",colnames(samp_inv))
str(samp_inv)

samp_inv_num <- samp_inv
# Let's order them:
# REF-Iceland = 1
# REF-GoM = 2
# HET-Iceland = 4
...
levels(samp_inv_num$LG1)
levels(samp_inv_num$LG1) <- c(7,4,1,5,2)

levels(samp_inv_num$LG2)
levels(samp_inv_num$LG2) <- c(8, 4, 2, 1, 7, 5)

levels(samp_inv_num$LG7) 
levels(samp_inv_num$LG7) <- c(4, 8, 5, 7, 2, 1)

levels(samp_inv_num$LG12)
levels(samp_inv_num$LG12) <- c(4, 5, 1, 8, 2, 7)

head(samp_inv_num)

samp_inv_num <- matrix(as.numeric(as.matrix(samp_inv_num)), ncol=4)

rownames(samp_inv_num) <- samp_full$label
colnames(samp_inv_num) <- colnames(samp_inv)
str(samp_inv_num)

 samp_ann <- cbind(
    Region = samp_full$Region_color,
    Pop = samp_full$PopID_color,
    Ecotype = samp_full$Ecotype_color
    )
  
  # for annotation in heatmap, the rownames of the annotation table has to have the same rownames as the matrix
  rownames(samp_ann) <- rownames(samp_inv_num)
  head(samp_ann)
  dim(samp_ann)
  
  heatmap_colors <- colorRampPalette(c("cornflowerblue", "grey", "firebrick3"))(8)
  
  
  pdf("figures/multi-inv-plot-v2.pdf", width=9, height=9)
  par(oma=c(0,0,0, 10))
  heatmap3(samp_inv_num,
           Rowv=NA,
           Colv = NA,
           showColDendro = F,
           showRowDendro = T,
...
```{r}
traits <- read.csv("allSample_morphometrics-uncorrected.csv")
head(traits)
table(traits$Location,traits$Ecotype)

# align "trait" data with samples in genotype matrix ####
traits$ID <- NA
traits$ID <- paste0(substr(traits$SampleID,1,2), substr(traits$SampleID,4,6))
head(traits$ID)
tail(traits$ID)
dim(traits)
...
samp_full2 <- merge(samp_full, traits, 
                    by.x = "samp_full", 
                    by.y = "ID", all.x=TRUE)

samp_full2 <- samp_full2[order(samp_full2$X),]
# check the dimensions
if (dim(samp_full)[1] != dim(samp_full2)[1]) break()
if(!identical(samp_full$samp_full, samp_full2$samp_full)) break()

# Examine samples with genotypes but missing traits
missing <- is.na(samp_full2$D1)
sum(missing)
samp_full2$label[which(missing)]
...
# check the sample dataframe is identical
if (!identical(samp_full2$label, samp)) break()

dim(G_LGX)
all_gen <- G_LGX[samprows,]
  
rda_lgx <- rda(all_gen~rda_traits)

scores <- scores(rda_lgx)
str(scores)

samp_full2[samprows,paste0(LGtext[i],"RDA1")] <- scores$sites[,1]
samp_full2[samprows,paste0(LGtext[i],"RDA2")] <- scores$sites[,2]
...
rownames(bp) <- c("D1", "D2", "D3", 
                  "D4", "D5", "D16")
bp$names <- rownames(bp)
head(bp)


g <- ggplot() + ggtheme + 
  geom_point(data=samp_full2, 
